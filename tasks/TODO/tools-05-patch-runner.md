...
Dependencies:
- Depends on TOOLS-00 (`ForegroundProcessRunner`, `ProcessSpec`, `ProcessRunResult`).

Scope:
- Create `src/CodingAgent/Tool/PatchRunner.php`.
- Wrap GNU `patch` via `ForegroundProcessRunner` rather than owning a custom Process loop.
...
- Use the shared timeout and foreground-process registration behavior from `ForegroundProcessRunner`; cancellation termination is handled by the controller/runtime registry path from TOOLS-00.
...
- Patch subprocess timeout uses `ForegroundProcessRunner` semantics, and cancellation uses the TOOLS-00 foreground process registry/terminator path.
