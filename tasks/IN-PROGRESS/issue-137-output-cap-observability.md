...
## Task workflow update - 2026-06-18T16:58:56.334Z
- Summary: User rejected PR #164/current branch architecture as a no-go. Decision: stop iterating on the current output-cap-specific plumbing. The architecture should be reset/rethought around a generalized model-notification system usable by output cap, SafeGuard, extensions, and any future model-facing nudge/notification. Output cap should become a client/user of that generic system rather than piercing output-cap-specific fields through runtime/TUI/projection layers. Current branch should be treated as rejected/reference-only until explicit rollback/restart instructions are given.
- Architecture rejection recorded: current implementation is considered half-dependent spaghetti that pierces output-cap concerns through the whole system.
- Reusable learning: freeze the generalized abstraction before implementation. The right primitive is likely a structured model notification/message system independent of OutputCap, not output_cap-specific metadata threaded through every layer.
