...
## Task workflow update - 2026-05-19T00:30:15.921Z
- Moved IN-PROGRESS → CODE-REVIEW.
- Pushed task/ai-14-tui-model-controls-favorites to origin.
- branch 'task/ai-14-tui-model-controls-favorites' set up to track 'origin/task/ai-14-tui-model-controls-favorites'.
- Skipped PR creation (pushOnly: true).
- Validation: vendor/bin/phpunit tests/CodingAgent/Config/ModelSelectionServiceTest.php --filter "Display|Supported" — 9 tests, OK; vendor/bin/phpunit tests/Tui/Picker/ModelPickerControllerTest.php — 7 tests, OK; vendor/bin/phpunit tests/Tui/Listener/ModelCommandHandlerTest.php — 18 tests, OK; ~/.local/bin/castor test — 537 tests, 8541 assertions, OK (1 pre-existing notice); ~/.local/bin/castor deptrac — 0 violations; ~/.local/bin/castor phpstan — 0 errors; ~/.local/bin/castor cs-check — clean
- Summary: AI-14 picker UX/config follow-up complete at commit 3e091136. Removed xhigh from z.ai settings/defaults/docs and made reasoning cycling per-model via thinking_level_map keys + off, so z.ai no longer exposes xhigh while DeepSeek keeps it. Styled picker headers with muted theme color, colored current/favorite markers using ThemeColor Accent/Warning, removed textual current description, and cleaned favorites header wording. Added ChatScreen::theme() to pass runtime theme into picker label/header rendering.

## Task workflow update - 2026-05-19T00:39:14.781Z
- Summary: Reviewer subagent completed AI-14 PR #27 review at head 3e091136. Verdict: APPROVE WITH SUGGESTIONS. No critical issues, no wheel reinvention detected, Symfony TUI reuse deemed appropriate (SelectListWidget, ContainerWidget, TextWidget, Keybindings, events/lifecycle). SOLID/architecture boundaries acceptable with clear separation of ModelSelectionService, picker controllers, command handler, and footer provider. Findings: duplicate lookupContextWindow logic in ModelPickerController/ModelControlListener/ModelCommandHandler; test bug in ModelPickerControllerTest where trim() masks non-favorite marker assertion; HomeSettingsWriter regex comment overwrite risk; stale comment in ModelPickerController referencing removed current description; unused $pickerController variable in ModelControlListener; optional docblock for public closure callback methods.

## Task workflow update - 2026-05-19T00:54:52.192Z
- Moved CODE-REVIEW → IN-PROGRESS.
- Summary: Moving AI-14 back to IN-PROGRESS to address reviewer suggestions: deduplicate lookupContextWindow, fix non-favorite marker test using trim(), update stale ModelPickerController comment, remove unused ModelControlListener variable, and consider HomeSettingsWriter commented-key overwrite risk.

## Task workflow update - 2026-05-19T01:05:00.790Z
- Moved IN-PROGRESS → CODE-REVIEW.
- Pushed task/ai-14-tui-model-controls-favorites to origin.
- branch 'task/ai-14-tui-model-controls-favorites' set up to track 'origin/task/ai-14-tui-model-controls-favorites'.
- Skipped PR creation (pushOnly: true).
