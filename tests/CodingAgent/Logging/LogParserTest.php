...
final class LogParserTest extends TestCase
{
    ...
    public function testParseValidJsonLine(string $line, array $expected): void
    {
        ...
        $this->assertSame($expected['channel'], $entry->channel);
        $this->assertSame($expected['level'], $entry->level);
        $this->assertSame($expected['message'], $entry->message);
        $this->assertSame('test.log', $entry->sourceFile);
        $this->assertSame(1, $entry->lineNumber);
    }
    ...
    public function testParseHandlesExtraAndContext(): void
    {
        ...
        $this->assertSame(['user_id' => 42, 'action' => 'login'], $entry->context);
        $this->assertSame(['ip' => '127.0.0.1'], $entry->extra);
    }
    ...
    {
        $this->assertNull($this->parser->parse('not valid json'));
        $this->assertNull($this->parser->parse('{broken'));
    }
    ...
    {
        $this->assertNull($this->parser->parse('"just a string"'));
        $this->assertNull($this->parser->parse('42'));
    }
    ...
    public function testParseAcceptsStringDatetime(): void
    {
        ...
        $this->assertNotNull($entry);
        $this->assertSame('2026-05-20T15:30:00+00:00', $entry->datetime->format(\DateTimeInterface::ATOM));
    }
    ...
    public function testParseAcceptsIntegerTimestamp(): void
    {
        ...
        $this->assertNotNull($entry);
        $this->assertSame($ts, $entry->datetime->getTimestamp());
    }
    ...
    public function testParseSourceFileAndLineNumberOptional(): void
    {
        ...
        $this->assertNotNull($entry);
        $this->assertNull($entry->sourceFile);
        $this->assertNull($entry->lineNumber);
    }
