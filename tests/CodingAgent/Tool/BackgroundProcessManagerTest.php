...
final class BackgroundProcessManagerTest extends IsolatedKernelTestCase
{
    ...
    public function testFindByRecordIdRebindsDeadLauncherPidToLiveWrapper(): void
    {
        $this->createManager(stopGraceSeconds: 0);

        $ready = $this->tmpDir.'/rebind-ready';
        $hold = $this->tmpDir.'/rebind-hold';
        $command = \sprintf(
        ...
        $started = $this->manager->start($command, self::TEST_SESSION);
        ...
        try {
            ...
            $entity = $this->manager->findByRecordId($started->id, self::TEST_SESSION);
            $this->assertNotNull($entity);
            $this->assertSame('running', $entity->status->value);

            $pidPath = substr($entity->statusPath, 0, -7).'.pid';
            $wrapperPid = (int) trim((string) file_get_contents($pidPath));
            $this->assertSame($wrapperPid, $entity->pid);

            // Simulate the historical false-unclean failure mode: DB still
            // points at a dead launcher PID while the wrapper PID file and
            // workload remain alive.
            $fakeLauncherPid = $wrapperPid + 100000;
            while (is_dir('/proc/'.$fakeLauncherPid)) {
                ++$fakeLauncherPid;
            }

            $entity->pid = $fakeLauncherPid;
            $entity->pgid = $fakeLauncherPid;
            $entity->finishedAt = null;
            $entity->exitCode = null;
            $entity->status = \Ineersa\CodingAgent\Entity\BackgroundProcessStatusEnum::Running;
            static::getContainer()->get(ProcessStore::class)->flush();

            $rebound = $this->manager->findByRecordId($started->id, self::TEST_SESSION);
            $this->assertNotNull($rebound);
            $this->assertSame('running', $rebound->status->value);
            $this->assertSame($wrapperPid, $rebound->pid);
            $this->assertNull($rebound->finishedAt);

            $events = array_column(
                array_filter(
                    $this->logger->records,
                    static fn (array $record): bool => 'background_process.pid_rebound' === $record['message'],
                ),
                'context',
            );
            $this->assertNotEmpty($events);
            $this->assertSame($wrapperPid, $events[0]['process_pid'] ?? null);
            $this->assertSame($fakeLauncherPid, $events[0]['previous_pid'] ?? null);
            $this->assertSame($entity->logPath, $events[0]['log_path'] ?? null);
        } finally {
    ...
    public function testTrueUncleanExitStillPersistsWhenWrapperAndStatusAreGone(): void
    {
        $this->createManager(stopGraceSeconds: 0);

        $started = $this->manager->start('echo gone', self::TEST_SESSION);
        $this->waitUntilFinished($started->pid);
        ...
        $pidPath = substr($entity->statusPath, 0, -7).'.pid';
        @unlink($entity->statusPath);
        @unlink($pidPath);

        $entity->finishedAt = null;
        $entity->exitCode = null;
        $entity->status = \Ineersa\CodingAgent\Entity\BackgroundProcessStatusEnum::Running;
        $entity->pid = 1_000_000_007;
        static::getContainer()->get(ProcessStore::class)->flush();

        $resolved = $this->manager->findByRecordId($started->id, self::TEST_SESSION);
        $this->assertNotNull($resolved);
        $this->assertSame('finished (unclean)', $resolved->status->value);
        $this->assertNull($resolved->exitCode);
        $this->assertNotNull($resolved->finishedAt);
