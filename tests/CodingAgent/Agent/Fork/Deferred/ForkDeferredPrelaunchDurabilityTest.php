...
{
    public function testStructuralCompactionNoOpAfterReplayStillSanitizesForkLaunchAndStartsChildOnce(): void
    {
        ...
        $replayed = $rebuilder->rebuildIfStale($forkState, $forkLocalRunId)->state;
        $this->assertFalse($this->messagesContainForkToolCall($replayed->messages));
        $this->assertSame([], $replayed->pendingToolCalls);

        $container->get(ForkSessionCopyService::class)->removeForkLocalSession($forkLocalRunId);
    }

    public function testContinueDispatchFailureAllowsRetryWithoutDuplicateChildStart(): void
    {
        $container = self::getContainer();
        $innerBus = $container->get(MessageBusInterface::class);
        $container->set(MessageBusInterface::class, new DurabilityFailingOnceContinueBus($innerBus));
        $container->set(AgentRunnerInterface::class, new DurabilityNoOpCompactRunner());
