...
final class BackgroundProcessCompletionPollerTest extends IsolatedKernelTestCase
{
    ...
    public function testPollSendsFollowUpAndMarksNotified(): void
    {
        ...
        self::assertSame('test-poller-run', $runId);
        self::assertSame('follow_up', $command->type);
        self::assertStringContainsString('[BG_PROCESS_DONE]', $command->text ?? '');
        self::assertStringContainsString('PID 10001', $command->text ?? '');
        self::assertStringContainsString('Hello world', $command->text ?? '');

        $entity = $this->findProcess(10001);
        self::assertNotNull($entity);
        self::assertNotNull($entity->completionNotifiedAt);
    }
    ...
    public function testPollSkipsNonBackgrounded(): void
    {
        ...
        self::assertEmpty($this->sentCommands);

        // Verify NOT marked notified
        $entity = $this->findProcess(20001);
        self::assertNotNull($entity);
        self::assertNull($entity->completionNotifiedAt);
    }
    ...
    public function testLogNotFoundStillSendsNotification(): void
    {
        ...
        self::assertStringContainsString('log file not found', $command->text ?? '');
        self::assertStringContainsString('exit 1', $command->text ?? '');
    ...
    public function testEmptyStringSessionIdNormalizedToNull(): void
    {
        ...
        self::assertNull($ref->getValue($poller));
    }
    ...
    public function testRegressionFreshBackgroundedProcessWithoutFinishedAt(): void
    {
        ...
        self::assertCount(1, $this->sentCommands);
        [$runId, $command] = $this->sentCommands[0];
        self::assertSame('test-regression-run', $runId);
        self::assertSame('follow_up', $command->type);
        self::assertStringContainsString('[BG_PROCESS_DONE]', $command->text ?? '');
        self::assertStringContainsString('PID '.$pid, $command->text ?? '');
        self::assertStringContainsString('regression output', $command->text ?? '');

        // Assert DB state was updated by the refresh
        $entity = $this->findProcess($pid);
        self::assertNotNull($entity);
        self::assertNotNull($entity->finishedAt, 'finishedAt must be populated after refresh');
        self::assertSame(BackgroundProcessStatusEnum::Finished, $entity->status);
        self::assertNotNull($entity->completionNotifiedAt, 'completionNotifiedAt must be set after notification');
    }
