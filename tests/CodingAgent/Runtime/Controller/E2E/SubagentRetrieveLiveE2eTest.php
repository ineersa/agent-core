...
MD;
    ...
    public function testSubagentThenAgentRetrieveChain(): void
    {
        ...
        self::assertArrayHasKey('run.started', $subagentByType,
        ...
        self::assertArrayHasKey('tool_execution.started', $subagentByType,
        ...
        self::assertNotEmpty($subagentCallId);
        ...
        self::assertNotNull($subagentCompleted, 'Missing matching tool_execution.completed for subagent. '
            .$this->collectDiagnostics($subagentEvents));

        self::assertFalse($subagentCompleted['is_error'] ?? true,
        ...
        self::assertNotSame('', $subagentResult, 'subagent completed payload must include result. '
            .$this->collectDiagnostics($subagentEvents));

        self::assertStringContainsString(self::HANDOFF_TOKEN, $subagentResult,
            'subagent result must include child handoff token. '.$this->collectDiagnostics($subagentEvents));
        self::assertStringContainsString('Subagent '.self::CHILD_AGENT.' completed.', $subagentResult,
        ...
        if (1 !== preg_match('/Artifact: (agent_[0-9a-f]{16})/', $subagentResult, $matches)) {
            self::fail('subagent result must include Artifact: agent_<16 hex>. Result: '.$subagentResult."\n"
                .$this->collectDiagnostics($subagentEvents));
        }
        ...
        self::assertStringContainsString($artifactId, $registryRaw,
        ...
        self::assertTrue($this->foundAck($retrieveEvents, $followUpCmdId),
        ...
        self::assertNotNull($retrieveStarted, 'Expected agent_retrieve tool_execution.started. '
            .$this->collectDiagnostics($retrieveEvents));

        $retrieveCallId = (string) ($retrieveStarted['payload']['tool_call_id'] ?? '');
        self::assertNotEmpty($retrieveCallId);

        $retrieveCompleted = $this->findCompletedPayloadForCallId($retrieveEvents, $retrieveCallId);
        self::assertNotNull($retrieveCompleted, 'Missing matching tool_execution.completed for agent_retrieve. '
            .$this->collectDiagnostics($retrieveEvents));

        self::assertFalse($retrieveCompleted['is_error'] ?? true,
        ...
        self::assertStringContainsString(self::HANDOFF_TOKEN, $retrieveResult,
            'agent_retrieve handoff must include child token. '.$this->collectDiagnostics($retrieveEvents));
        self::assertStringNotContainsString($this->tempDir, $retrieveResult,
