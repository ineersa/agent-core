...
/**
 * Thesis: stock Doctrine redeliver_timeout=315360000 (~ten 365-day years) keeps
 * claimed rows unreceivable after 2h/1d aging, still delivers fresh unclaimed
 * rows, and reclaims only after delivered_at is older than that horizon. Ack and
 * reject delete claimed rows. Explicit SessionRepairService redrive enqueues a
 * fresh receivable envelope and leaves the abandoned claimed row untouched. No
 * keepalive or wall-clock wait; horizon reclaim uses a backdated delivered_at.
 *
 ...
 *
 * @covers \Symfony\Component\Messenger\Bridge\Doctrine\Transport\Connection
 */
final class MessengerDoctrineRedeliverTimeoutLeaseTest extends IsolatedKernelTestCase
{
    ...
    public function testClaimedRowStaysUnreceivableWithinHorizonAndReclaimsPastIt(): void
    {
        [$transport, $fresh, $queueName] = $this->openStockTransport('lease');
        ...
        try {
            ...
            $twoHoursAgo = (new \DateTimeImmutable('UTC'))->modify('-7200 seconds')->format('Y-m-d H:i:s');
            $updated = $fresh->executeStatement(
                'UPDATE messenger_messages SET delivered_at = ? WHERE queue_name = ? AND delivered_at IS NOT NULL',
                [$twoHoursAgo, $queueName],
            );
            $this->assertSame(1, $updated, 'Exactly one delivered row must be aged two hours.');

            $agedTwoHours = iterator_to_array($transport->get());
            $this->assertSame([], $agedTwoHours, 'Two-hour claimed row must remain unreceivable within the ten-year horizon.');

            $oneDayAgo = (new \DateTimeImmutable('UTC'))->modify('-86400 seconds')->format('Y-m-d H:i:s');
            $updated = $fresh->executeStatement(
                'UPDATE messenger_messages SET delivered_at = ? WHERE queue_name = ? AND delivered_at IS NOT NULL',
                [$oneDayAgo, $queueName],
            );
            $this->assertSame(1, $updated, 'Exactly one delivered row must be aged one day.');

            $agedOneDay = iterator_to_array($transport->get());
            $this->assertSame([], $agedOneDay, 'One-day claimed row must remain unreceivable within the ten-year horizon.');
            ...
            $this->assertSame(
                ...
                'The original aged claimed row must remain until explicitly deleted or past-horizon reclaim.',
            );

            $pastHorizon = (new \DateTimeImmutable('UTC'))
                ->modify(\sprintf('-%d seconds', self::REDELIVER_TIMEOUT_SECONDS + 1))
                ->format('Y-m-d H:i:s');
            $updated = $fresh->executeStatement(
                'UPDATE messenger_messages SET delivered_at = ? WHERE queue_name = ? AND delivered_at IS NOT NULL',
                [$pastHorizon, $queueName],
            );
            $this->assertSame(1, $updated, 'Exactly one delivered row must be aged past the ten-year horizon.');

            $reclaimed = iterator_to_array($transport->get());
            $this->assertCount(1, $reclaimed, 'Claimed row older than redeliver_timeout must become receivable again.');
            ...
            $this->assertSame(
                ...
                'Ack after past-horizon reclaim must delete the row.',
            );
        } finally {
    ...
    public function testRepairRedriveEnqueuesFreshReceivableEnvelopeWithoutClearingClaimedRow(): void
    {
        ...
        [$transport, $fresh] = $this->openStockTransport($runId, $queueName);
     ...
     */
    private function openStockTransport(string $suffix, ?string $queueName = null): array
    {
        ...
        $this->assertSame(self::REDELIVER_TIMEOUT_SECONDS, (int) $configuration['redeliver_timeout']);
