...
final class SubagentLiveChildViewPollerReplayTest extends TestCase
{
    ...
    #[Test]
    public function replaySnapshotInvokesCatalogIngestForNestedProgress(): void
    {
        $projector = $this->childLiveProjector();
        $poller = new SubagentLiveChildViewPoller($projector, new NullLogger());
        $live = $this->liveState();
        $live->selected = new SubagentLiveChildDTO(
            agentRunId: 'fork-run-1',
            artifactId: 'agent_fork',
            agentName: 'fork',
            status: SubagentLiveStatusEnum::Running,
            taskSummary: 'delegate',
            lastActivityAtMs: 1,
        );
        ...
        $poller->replaySnapshot(
            $live,
            $snapshot,
            onCatalogIngest: static function (RuntimeEvent $event) use ($state, &$ingested): void {
                $state->subagentLiveCatalog->ingestNestedProgressFromChildRunEvent($event);
                $ingested[] = $event->seq;
            },
        );

        $this->assertSame([2], $ingested);
    ...
    #[Test]
    public function pollProjectsLiveSelectedRunEventAfterReplayCursor(): void
    {
        $projector = $this->childLiveProjector();
        $poller = new SubagentLiveChildViewPoller($projector, new NullLogger());
        $live = $this->liveState();
        ...
        $client->expects($this->once())
            ...
            ->willReturn([
                new RuntimeEvent(
                    ...
                    2,
                    [
                        'question_id' => 'q_live_tail',
                        'prompt' => 'live tail after replay',
                    ],
                ),
            ]);

        $blocks = $poller->poll($live, $client);
        $this->assertNotNull($blocks);
        $texts = array_map(static fn (TranscriptBlock $block): string => $block->text, $blocks);
        $this->assertContains('live tail after replay', $texts);
        $this->assertSame(2, $live->childLastSeq);
    }
