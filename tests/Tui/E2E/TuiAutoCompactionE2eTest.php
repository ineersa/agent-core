...
final class TuiAutoCompactionE2eTest extends TestCase
{
    ...
    public function testAutoCompactionTriggeredAndVisibleInTui(): void
    {
        ...
        try {
            ...
            $this->tmux->saveAnsiSnapshot($pane, 'auto-compact-success');

            // Clean exit.
            $this->tmux->sendKey($pane, 'C-d');
        } catch (\Throwable $e) {
            $this->tmux->saveAnsiSnapshot($pane, 'auto-compact-FAILURE');
            try {
                $this->tmux->sendKey($pane, 'C-d');
            } catch (\Throwable) {
            }
    ...
    private function agentCommand(): string
    {
        ...
        $paths = TuiE2eDatabaseEnv::allocatePaths('tui-auto-compact-');
     ...
     * of TuiCompactCommandVirtualTest (including SafeGuard extension config).
     */
     ...
     *
     * Matches TuiCompactCommandVirtualTest pattern exactly (includes
