...
final class TuiOutputCapNoticeE2eTest extends TestCase
{
    ...
    public function testOutputCapShowsNotificationAndHidesRawOutput(): void
    {
        ...
        try {
            ...
            $this->tmux->saveAnsiSnapshot($pane, 'output-cap-notification');

            // Clean exit.
            $this->tmux->sendKey($pane, 'C-d');
        } catch (\Throwable $e) {
            $this->tmux->saveAnsiSnapshot($pane, 'output-cap-notification-FAILURE');
            try {
                $this->tmux->sendKey($pane, 'C-d');
            } catch (\Throwable) {
            }
    ...
    private function agentCommand(): string
    {
        ...
        $paths = TuiE2eDatabaseEnv::allocatePaths('tui-output-cap-');
