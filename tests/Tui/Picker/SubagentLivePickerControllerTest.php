...
');
    ...
    #[Test]
    public function testParallelPendingAndRunningChildrenAppearAsDistinctPickerRows(): void
    {
        $harness = new VirtualTuiHarness(sessionId: 'picker-parallel-pending');
        $state = new TuiSessionState('picker-parallel-pending');
        $state->subagentLiveCatalog->ingestRuntimeEvent(new RuntimeEvent(
            type: RuntimeEventTypeEnum::ToolExecutionOutputDelta->value,
            runId: 'parent-run',
            seq: 1,
                ...
                'subagent_progress' => [
                    ...
                    'children' => [
                        [
                            ...
                            'agent_name' => 'scout',
                            'status' => 'pending',
                            'artifact_id' => 'agent_pending',
                            'agent_run_id' => 'child-pending',
                            'task_summary' => 'Queued scout',
                        ],
                        [
                            'index' => 2,
                            'agent_name' => 'reviewer',
                            'status' => 'running',
                            'artifact_id' => 'agent_running',
                            'agent_run_id' => 'child-running',
                            'task_summary' => 'Active reviewer',
                        ],
                    ],
                ],
            ],
        ...
        $labels = array_map(static function (array $item): string {
            ...
            return preg_replace('/\x1b\[[0-9;]*m/', '', $label) ?? $label;
        }, $items);
        $joined = implode("\n", $labels);
        $this->assertStringContainsString('agent_pending', $joined);
        $this->assertStringContainsString('agent_running', $joined);
        $this->assertStringContainsString('pending', strtolower($joined));
        $this->assertStringContainsString('running', strtolower($joined));
    }
