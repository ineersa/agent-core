...
final class LlmStepResultHandlerTest extends TestCase
{
    ...
    public function testAbortedDoesNotAppendAssistantMessageToState(): void
    {
        ...
        $state = new RunState(
            ...
            currentOperation: new CurrentOperationDTO(1, 'abort-step', 1, 'llm-abort-1'),
    ...
    public function testAbortedWithOnlyTextDoesNotAppendMessage(): void
    {
        ...
        $state = new RunState(
            ...
            currentOperation: new CurrentOperationDTO(1, 'abort-text-step', 1, 'llm-abort-text-1'),
            messages: $existingMessages,
            model: 'test-model');
    ...
    public function testStopBoundaryMailboxEffectsContainPendingCompact(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 4,
            activeStepId: 'turn-1-step',
            currentOperation: new CurrentOperationDTO(1, 'turn-1-step', 1, 'llm-no-tools-1'),
            model: 'test-model');
    ...
    public function testErrorResultDoesNotAppendAssistantMessage(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 4,
            activeStepId: 'turn-1-step',
            currentOperation: new CurrentOperationDTO(1, 'turn-1-step', 1, 'empty-asst-1'),
            messages: $existingMessages,
            model: 'test-model');
    ...
    public function testRetryableErrorBelowCapSchedulesAutomaticContinue(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 3,
            activeStepId: 'step-1',
            currentOperation: new CurrentOperationDTO(1, 'step-1', 1, 'llm-retry-1'),
            messages: [
        ...
        $this->assertSame(3, $failed->payload['max_retries'] ?? null);
        $this->assertSame(
            [RunEventTypeEnum::LlmStepFailed->value],
        ...
        $delay = $commandBus->envelopes[0]->last(DelayStamp::class);
        $this->assertInstanceOf(DelayStamp::class, $delay);
        $this->assertSame(1000, $delay->getDelay());
    ...
    #[DataProvider('agentRetryDelayProvider')]
    public function testRetryableErrorSchedulesExponentialDelayStampBackoff(int $currentAttempts, int $expectedDelayMs): void
    {
        $commandBus = new TestMessageBus();
        $classifier = new \Ineersa\AgentCore\Infrastructure\SymfonyAi\LlmProviderErrorClassifier();
        ...
        $handler = new LlmStepResultHandler(
            ...
            commandBus: $commandBus,
            errorClassifier: $classifier,
            agentRetryMaxAttempts: 5,
        );

        $state = new RunState(
            runId: 'run-retry-delay',
            status: RunStatus::Running,
            version: 2,
            turnNo: 1,
            lastSeq: 3,
            activeStepId: 'step-delay',
            currentOperation: new CurrentOperationDTO(1, 'step-delay', 1, 'llm-delay-1'),
            messages: [
                new \Ineersa\AgentCore\Domain\Message\AgentMessage(role: 'user', content: [['type' => 'text', 'text' => 'Hi']]),
            ],
        ...
        $delay = $commandBus->envelopes[0]->last(DelayStamp::class);
        $this->assertInstanceOf(DelayStamp::class, $delay);
        $this->assertSame($expectedDelayMs, $delay->getDelay());
    }

    public static function agentRetryDelayProvider(): array
    {
        return [
            'first retry' => [0, 1000],
            'second retry' => [1, 2000],
    ...
    public function testRetryableErrorAtCapDoesNotDispatchRetry(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 3,
            activeStepId: 'step-1',
            currentOperation: new CurrentOperationDTO(1, 'step-1', 1, 'llm-exhausted-1'),
            messages: [
                new \Ineersa\AgentCore\Domain\Message\AgentMessage(role: 'user', content: [['type' => 'text', 'text' => 'Hi']]),
            ],
            retryAttempts: 3,
        ...
        $this->assertSame(
            'Automatic LLM retry attempts exhausted after 3 retry attempt(s): LLM provider server error interrupted the response stream.',
            $errorMessage,
        );
        $this->assertNull($result->nextState->activeStepId);
        $this->assertNull($result->nextState->currentOperation);
        ...
        $this->assertSame(
            [RunEventTypeEnum::LlmStepFailed->value, RunEventTypeEnum::AgentEnd->value],
            array_map(static fn ($event): string => $event->type, $result->events),
        );
        ...
        $this->assertSame(3, $failed->payload['retry_attempt'] ?? null);
        $this->assertSame(3, $failed->payload['max_retries'] ?? null);
        ...
        $this->assertSame(3, $logger->records[0]['context']['retry_attempt'] ?? null);
        $this->assertSame(3, $logger->records[0]['context']['max_retries'] ?? null);
    ...
    public function testContextOverflowFailsVisiblyWithoutCompactRunOrAutoRetry(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 3,
            activeStepId: 'step-1',
            currentOperation: new CurrentOperationDTO(1, 'step-1', 1, 'llm-overflow-1'),
            messages: [
                new \Ineersa\AgentCore\Domain\Message\AgentMessage(role: 'user', content: [['type' => 'text', 'text' => 'Hi']]),
            ],
            model: 'test-model');
    ...
    public function testNonRetryableErrorEmitsTerminalAgentEndWithoutRetry(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 3,
            activeStepId: 'step-1',
            currentOperation: new CurrentOperationDTO(1, 'step-1', 1, 'llm-nonretryable-1'),
            messages: [
                new \Ineersa\AgentCore\Domain\Message\AgentMessage(role: 'user', content: [['type' => 'text', 'text' => 'Hi']]),
            ],
            model: 'test-model',
        ...
        $result = $handler->handle(new LlmStepResult(
            ...
            stepId: 'step-1',
            attempt: 1,
            idempotencyKey: 'llm-nonretryable-1',
            assistantMessage: null,
            usage: [],
            stopReason: null,
            error: $error,
    ...
    public function testParallelToolCallsCarryConfiguredMaxParallelism(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 4,
            activeStepId: 'turn-1-step',
            currentOperation: new CurrentOperationDTO(1, 'turn-1-step', 1, 'llm-parallel-max'),
    ...
    public function testExecuteToolCallHasNullTimeoutWithoutPerToolOverride(): void
    {
        ...
        $state = new RunState(
            ...
            lastSeq: 1,
            activeStepId: 'step-1',
            currentOperation: new CurrentOperationDTO(1, 'step-1', 1, 'llm-timeout-null'),
            model: 'test-model');
