...
final class ExecuteLlmStepWorkerTest extends TestCase
{
    ...
    public function testTwoWorkersOverlapAtPlatformInvokeBarrier(): void
    {
        $projectRoot = \dirname(__DIR__, 4);
        $autoload = $projectRoot.'/vendor/autoload.php';
        $script = __DIR__.'/Support/execute_llm_step_barrier_worker.php';
        $this->assertFileExists($autoload);
        $this->assertFileExists($script);

        $tmp = \Ineersa\CodingAgent\Tests\Support\TestDirectoryIsolation::createProjectTempDir('llm-step-barrier');
        $gatePath = $tmp.'/gate';
        $enteredDir = $tmp.'/entered';
        mkdir($enteredDir, 0755, true);
        ...
        try {
            ...
            if (!flock($gateHandle, \LOCK_EX)) {
                $this->fail('Failed to acquire exclusive barrier gate.');
            }

            $p1 = new \Symfony\Component\Process\Process(['php', $script, $autoload, 'w1', $gatePath, $enteredDir]);
            $p2 = new \Symfony\Component\Process\Process(['php', $script, $autoload, 'w2', $gatePath, $enteredDir]);
            $p1->setTimeout(10);
            $p2->setTimeout(10);
            $p1->start();
            $p2->start();
            ...
            while (microtime(true) < $deadline) {
                if (is_file($enteredDir.'/w1.entered') && is_file($enteredDir.'/w2.entered')) {
                    break;
                }
                if (!$p1->isRunning() || !$p2->isRunning()) {
                    $this->fail(\sprintf(
                        'Worker exited before both entered invoke. p1=%s p2=%s out1=%s err1=%s out2=%s err2=%s',
                        $p1->getExitCode(),
                        $p2->getExitCode(),
                        $p1->getOutput(),
                        $p1->getErrorOutput(),
            ...
            $this->assertFileExists($enteredDir.'/w1.entered', 'Worker w1 must enter PlatformInterface::invoke before release.');
            $this->assertFileExists($enteredDir.'/w2.entered', 'Worker w2 must enter PlatformInterface::invoke before release.');
            $this->assertTrue($p1->isRunning(), 'Worker w1 must still be blocked on the gate while overlapping.');
            $this->assertTrue($p2->isRunning(), 'Worker w2 must still be blocked on the gate while overlapping.');
            ...
            if ($p1 instanceof \Symfony\Component\Process\Process && $p1->isRunning()) {
                $p1->stop(0);
            }
            if ($p2 instanceof \Symfony\Component\Process\Process && $p2->isRunning()) {
                $p2->stop(0);
            }
            \Ineersa\CodingAgent\Tests\Support\TestDirectoryIsolation::removeDirectory($tmp);
        }
    }
