...
Release splitting mirrors the public Extension API and four project extensions into read-only GitHub/Packagist-facing repositories (`hatfield-extension-api`, `hatfield-ext-task-workflow`, `hatfield-ext-file-rewind`, `hatfield-ext-observational-memory`, `hatfield-ext-jbcontext`). Package READMEs travel with those mirrors. Core `hatfield_docs` does **not** auto-discover installed extension docs.
