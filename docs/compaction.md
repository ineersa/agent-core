...
- **Transport:** Hatfield-managed single FIFO `extension_agent` queue, `max_retries: 1`, no failure transport. Exhausted jobs emit sanitized `extension_agent.job_failed` TUI Error blocks when `run_id` is present.
