...
- **Conversation boundary hooks** (`AfterConversationBoundaryHookInterface`) fire best-effort after a terminal `agent_end` (`completed` / `failed` / `cancelled`) is persisted with allocated seq. They receive a public `ConversationBoundaryDTO` with run/session identity, opaque boundary id, inclusive source range, latest committed seq, and outcome. They must not mutate run state or perform model work. Delivery is an acceleration path, not a durability guarantee. `boundaryAt` is the terminal event `createdAt` when the just-persisted batch carries it (AfterTurnCommit summaries now include `turnNo`/`createdAt`); worker-failure permanent appends pass the terminal event through intact.
- **Hot post-commit event batches** (`AfterTurnCommitHookContextDTO::$events`) expose just-persisted canonical events as full `SessionEventDTO` values (payload, `turnNo`, `createdAt`) from the commit context. Boundary notifications similarly include the current batch. Extensions should queue these hot batches and own their durable previous cursor; range start is `(previous_cursor + 1)..sourceEndSeq`. Hatfield does not reread `events.jsonl` on the per-boundary hot path and does not invent `sourceStartSeq`.
- **Recovery/catch-up event reader** (`ExtensionApiInterface::sessionEvents()`) returns immutable `SessionEventDTO` values for an inclusive `(startSeq, endSeq)` range. Source identity is `(run_id, seq)`. Reads are intentionally non-branch-aware and may include abandoned rewind activity present in the journal. This is recovery/compaction access only — the MVP implementation may scan the full journal and must not be used every turn/boundary. Missing sessions raise a stable `SessionEventReaderException` distinct from an empty range on an existing session.
- **Blocking model call** (`ExtensionApiInterface::callModel(AiModelReference $model, MessageBag $messages, ?ToolboxInterface $toolbox = null): ResultInterface`) creates a standard non-streaming Symfony AI `Agent` on Hatfield's container-configured lazy `PlatformInterface`. When a toolbox is supplied, Symfony's `AgentProcessor` is attached as input and output processor so the normal tool loop runs. Ambient Hatfield tools are never injected. Native Symfony AI results and exceptions propagate; structured-output helpers and parallel public error/result DTOs are out of scope for MVP.
