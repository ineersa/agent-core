...
- New and resumed active runs perform zero `state.json` reads or writes. Read-only benchmark evidence measured the first replayable parent (44 events, 33 ms, 0-byte post-baseline peak delta) and child (4 events, 2 ms, 0-byte post-baseline peak delta) after container boot; no PHP memory-limit change is made from this evidence.
