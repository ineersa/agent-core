...
3. **Present the implementation plan**
   ...
   - **Suggested validation** — which `castor` commands to run after implementation. For TUI tasks: explicitly include the real TmuxHarness + test LLM E2E test that will prove the feature works, and note that `castor test:tui` is required before CODE-REVIEW.
