...
| `InProcessAgentSessionClient::events()` | Calls `allFor()` on every active poll; process-local `(size,mtime)` cache avoids unchanged decodes but every append invalidates it | Deferred to `TODO/storage-reduce-active-event-log-read-and-payload-amplification.md`. |
...
The completed smallest concrete-store optimization is bounded child `latestSequenceFor()`/`reverseFor()` using the existing durable reverse-line reader, without a cache/index/sidecar. The remaining active-read work is limited to removing `allFor()` from in-process active polling and replacing child `readAfterSeq()` zero-offset recovery scans; both are deferred to `TODO/storage-reduce-active-event-log-read-and-payload-amplification.md`. Keep canonical full reads at the explicit lifecycle boundaries above. `sequence.cursor` remains unusable as durable-tail truth because allocation can precede a failed append and leave valid gaps.
