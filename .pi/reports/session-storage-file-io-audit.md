...
Static writer multiplicity is similarly structural: `SessionRunEventStore::append`/`appendMany` and child equivalents each delegate to one locked `JsonlRunEventLog::appendMany`; `SessionRunStore::compareAndSwap` is the sole parent-state writer; `FileRunSequenceAllocator::allocateBlock` is the cursor writer. Their caller frequency is not observable from this snapshot. Readers are these stores plus the full-history consumers listed above; `SessionRunStore::get` is used both directly and inside CAS/stale scanning.
