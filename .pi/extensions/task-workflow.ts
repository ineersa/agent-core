...
const TASK_CONTEXT_TYPE = "task-context";
...
function createTaskContextState(): TaskContextState {
	return {
		lastHash: undefined,
...
async function buildTaskContextMessage(root: string): Promise<string | null> {
	const activeStatuses: TaskStatus[] = ["IN-PROGRESS", "CODE-REVIEW"];
	const allActive: { task: TaskInfo; text: string }[] = [];
	...
	for (const status of activeStatuses) {
		...
		for (const task of tasks) {
			let text = "";
			try {
				text = await readFile(task.path, "utf8");
			} catch {
				// best-effort
			}
			allActive.push({ task, text });
		}
	}

	if (allActive.length === 0) return null;

	// Determine workflow phase from highest-priority active status
	const hasInProgress = allActive.some(({ task }) => task.status === "IN-PROGRESS");
	const hasCodeReview = allActive.some(({ task }) => task.status === "CODE-REVIEW");
	const phase = hasInProgress ? "implementation" : "review";

	const lines: string[] = ["<current_task>"];

	for (const { task, text } of allActive) {
		lines.push(`Task: ${task.status}/${task.file}: ${task.title}`);
		...
		if (text) {
			...
			const accMatch = text.match(/^## Acceptance Criteria\s*\n((?:- .+\n?)*)/m);
			if (accMatch) lines.push(`Acceptance:\n${accMatch[1].trim()}`);
			// Extract latest work log entries (last 5)
			const logMatch = text.match(/^## Work Log\s*\n([\s\S]*?)(?=^## |$)/m);
			if (logMatch) {
				const entries = logMatch[1].trim().split("\n").filter((l) => l.startsWith("- "));
				lines.push("Recent log:");
				lines.push(...entries.slice(-5));
			}
		}
		lines.push("");
	}

	// Orchestrator role + phase boundary (no procedures — those live in prompts)
	lines.push(">");
	lines.push("");
	lines.push("You are in an active task workflow. You are an **orchestrator**, not an implementor.");
	lines.push("Use scouts for exploration, researchers for web lookup, and **forks for ALL implementation**.");
	lines.push("Never edit files directly — launch a fork instead.");

	if (phase === "implementation") {
		lines.push("");
		lines.push("Phase: Implementation. STOP after recording fork results.");
		lines.push("Do NOT: castor check, move_task(to=\"CODE-REVIEW\"), gh pr create, git push, reviewer.");
		lines.push("When done: inform user → they run task-to-pr.");
	} else if (phase === "review") {
		lines.push("");
		lines.push("Phase: Code Review. Do NOT merge or modify without explicit user request.");
		lines.push("Available prompts: task-to-pr, task-review-iterate, task-done.");
	}
...
export default function (pi: ExtensionAPI) {
	const state = createTaskContextState();
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		// On fork/resume: session already has our message, skip injection
		...
		try {
			...
			const content = await buildTaskContextMessage(root);
			const hash = content ? simpleHash(content) : "";
			...
			// Best-effort: if task board is unavailable, skip injection
			return { systemPrompt: prompt };
		}
	});
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Use move_task with to=\"CODE-REVIEW\" after the worktree branch is committed and ready for review; this pushes and creates a PR.",
