...
final class HiddenGitPerCommitRefRetentionTest extends TestCase
{
    ...
    public function testTwoCheckpointCommitsRemainReachableAfterThirdCapture(): void
    {
        ...
        $this->assertTrue($this->backend->commitShaReachable($this->hiddenGit, $this->projectDir, $commit1));
        $this->assertTrue($this->backend->commitShaReachable($this->hiddenGit, $this->projectDir, $commit2));
        $this->assertTrue($this->backend->commitShaReachable($this->hiddenGit, $this->projectDir, $commit3));
        $this->assertNotSame($commit1, $commit2);
    }
    ...
    public function testPruneRemovesRefsOutsideKeepSet(): void
    {
        ...
        $this->assertFalse($this->backend->commitShaReachable($this->hiddenGit, $this->projectDir, $commit1));
        $this->assertTrue($this->backend->commitShaReachable($this->hiddenGit, $this->projectDir, $commit2));
    }
}
