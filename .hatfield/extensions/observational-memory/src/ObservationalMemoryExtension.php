    ...
    public function register(ExtensionApiInterface $api): void
    {
        ...
        $api->registerTool(new ToolRegistrationDTO(
            ...
            promptGuidelines: [
                ...
                $hybrid ? 'Combine useful keywords or identifiers with a description of the concept or prior work. Hybrid search combines BM25 keyword search and semantic vector search; natural-language queries and paraphrases are supported.' : 'Pass one contiguous literal substring. Prefer a single identifier (for example 2510 or MapToolArguments). Multi-word queries match that exact phrase, not AND of separate words. Not natural-language questions, regex, or wildcard syntax.',
                $hybrid ? 'If there are no useful matches, reformulate the concept or add another known identifier or relevant detail.' : 'If there are no matches, try one word or another known term before concluding nothing exists.',
                $hybrid ? 'Defaults to all retained history; pass after/before only to narrow by memory date. Results are relevance-ranked from bounded candidates, not newest first. There is no total or pagination.' : 'Defaults to all retained history; pass after/before only to narrow by memory date. Results are newest first. If truncated, older matches were omitted; there is no total or pagination, so narrow the query or date range.',
