...
1. `move_task(to="IN-PROGRESS")`; this creates the task worktree and branch. Creation copies `vendor/` and `.vera/` and updates parent IDEA exclusions when present.
...
- Worktree lifecycle updates parent IDEA exclusions, creates minimal local IDEA metadata, and opens the exact worktree via MCP (`jetbrains-index_ide_open_project`). DONE/CANCELLED closes that project before removal. IDE degradation does not fail the transition. Prefer semantic IDE tools scoped to the exact worktree.
