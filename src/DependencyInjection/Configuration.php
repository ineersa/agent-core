        ...
        $rootNode
            ->children()
                ...
                ->arrayNode('llm')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('default_model')->isRequired()->cannotBeEmpty()->end()
                        ->enumNode('platform')->values(['openai', 'anthropic', 'generic', 'ollama', 'custom'])->defaultValue('openai')->end()
            ...
            ->end()
            ->validate()
                ->ifTrue(static fn (array $config): bool => !\is_string($config['llm']['default_model'] ?? null) || '' === trim($config['llm']['default_model']))
                ->thenInvalid('agent_loop.llm.default_model must be configured and non-empty.')
            ->end()
            ->validate()
