...
- **Extension agent jobs:** `extension_agent.job_failed` is JSONL/TUI-only (`seq=0`); does **not** append canonical RunEvents and does **not** alone mark the main run failed. Emit only when job `payload.run_id` is a validated non-empty scalar; missing `run_id` → structured log only. Its `message` contains the unwrapped extension handler exception message. Transcript projection uses a generic fallback when that message is empty.
...
- **Subagent elapsed_ms:** single snapshots and parallel child rows carry derived `elapsed_ms` from existing child `started_at` and `terminal_completed_at`. Unlaunched children use batch start. Forced interruption snapshots capture the clock when no child terminal timestamp exists. The emitted snapshot retains that value on replay. No new storage fields.
- **Tool timing:** translated execution starts carry `started_at`, and completed, failed, or cancelled executions carry `ended_at`. Both use canonical `RunEvent.createdAt` with microsecond precision. Projection preserves explicit `duration_ms` when present, otherwise it derives the final duration from these timestamps. Live TUI timers start at execution, not argument streaming.
