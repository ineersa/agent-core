...
- **Subagent elapsed_ms:** single snapshots and parallel child rows carry derived `elapsed_ms` from existing child `started_at` / `terminal_completed_at` (batch start as start fallback; live clock only while non-terminal). No new storage fields.
