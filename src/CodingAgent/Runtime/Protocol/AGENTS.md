...
- **Extension agent jobs:** `extension_agent.job_failed` is JSONL/TUI-only (`seq=0`); does **not** append canonical RunEvents and does **not** alone mark the main run failed. Emit only when job `payload.run_id` is a validated non-empty scalar; missing `run_id` → structured log only. Its message/reason fields come from fixed privacy-safe classifications, with the generic retry-exhausted fallback for unknown failures. Never include raw exception text, prompts, or tool output.
