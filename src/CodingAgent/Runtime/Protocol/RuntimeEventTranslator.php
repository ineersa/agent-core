...
final class RuntimeEventTranslator
{
    ...
    private function onLlmStepFailed(RunEvent $runEvent): RuntimeEvent
    {
        ...
        if (\is_array($error)) {
            foreach (['retryable', 'error_category', 'http_status_code', 'retry_after_ms', 'response_error_code', 'response_error_type', 'response_error_param', 'response_error_message'] as $key) {
