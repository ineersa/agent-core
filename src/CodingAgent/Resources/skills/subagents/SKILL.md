...
`agent_resume` continues an existing child only in the same parent session; it is not a fresh launch mechanism. Children cannot resume themselves or launch nested children. Fork children are not resumable: each new fork is a new worker and ownership handoff.
