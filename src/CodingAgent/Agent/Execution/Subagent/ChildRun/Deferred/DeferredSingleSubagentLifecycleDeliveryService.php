...
/**
 * Durable deferred single-child delivery: parent progress, artifact finalization, and generic deferred completion.
 */
final readonly class DeferredSingleSubagentLifecycleDeliveryService
{
    ...
    public function deliver(string $lifecycleId): void
    {
        ...
        if ($projection->childEventCursor > $projection->parentProgressCursor) {
            $this->emitParentProgress($projection, $childProjection, $expectedVersion);
            $row = $this->launchRepository->findEntityByLifecycleId($lifecycleId);
            if (null === $row) {
                return;
            }
        ...
        $this->completeTerminal($projection, $childProjection, $expectedVersion);
    }

    private function emitParentProgress(
        ...
        $progressStatus = $this->mapProgressStatus($childProjection->childStatus);
        ...
        $payload = $childProjection->childStatus->isTerminal()
    ...
    private function completeTerminal(
        DeferredSingleSubagentProjectionDTO $projection,
        DeferredSingleSubagentChildLifecycleProjectionDTO $childProjection,
        int $expectedProjectionVersion,
    ): void {
        ...
        $artifactOutcome = $this->buildArtifactOutcome($identity, $childProjection);
        $this->lifecycleListener->finalizeTerminalOutcome(
            ChildRunTerminalFinalizationRequestDTO::persistOnly($artifactOutcome),
        );

        $presentation = $this->buildPresentation($identity, $childProjection, $artifactOutcome);
    ...
    private function buildArtifactOutcome(
    ...
    private function mapProgressStatus(RunStatus $status): string
    {
        return match ($status) {
