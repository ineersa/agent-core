...
use Symfony\Component\Tui\Render\LayoutEngine;
 ...
 */
final class CachedWidthValidationRenderer implements WidgetRendererInterface
{
    ...
    /** @var \WeakMap<AbstractWidget, array{columns: int, lines: list<string>}> */
    private \WeakMap $validatedRows;
    ...
    {
        $this->validatedRows = new \WeakMap();
    ...
    public function renderWidget(AbstractWidget $widget, RenderContext $context): array
    {
        ...
        // Reuse proof for byte-identical rows at the same available width.
        // Revalidate on width changes or when a row's content changes.
        $validated = $this->validatedRows[$widget] ?? null;
        $previousLines = null !== $validated && $validated['columns'] === $availableColumns
            ? $validated['lines']
            : [];
        foreach ($lines as $i => $line) {
            if (isset($previousLines[$i]) && $previousLines[$i] === $line) {
                continue;
            }
        ...
        /** @var list<string> $validatedLines */
        $validatedLines = array_values($lines);
        $this->validatedRows[$widget] = ['columns' => $availableColumns, 'lines' => $validatedLines];
