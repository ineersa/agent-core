...
1. State the question and an explicit UTC/relative time range. Discover available guides with `list_datadog_skills` and load the relevant guide before unfamiliar query syntax.
2. Use searches to find raw evidence: `search_datadog_logs`, `search_datadog_spans`, `search_datadog_metrics`, `search_datadog_monitors`, `search_datadog_dashboards`, `search_datadog_incidents`, and service/dependency searches. Use aggregations (`analyze_datadog_logs`, `aggregate_spans`, `aggregate_events`) for counts, rates, or grouped trends instead of sampling raw events.
