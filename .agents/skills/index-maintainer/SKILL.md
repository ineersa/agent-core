...
description: Maintain the ai-index.json documentation system — create, update, or fix indexes and docs for any namespace, sub-namespace, or file. Use when adding/removing/renaming files, creating new namespaces, or when indexes are stale/incomplete. Also use when user mentions ai-index, docs maintenance, or codebase documentation.
model: zai/glm-5.1
---

# Index Maintainer

Maintains the hierarchical `ai-index.json` + `docs/` documentation system for the entire `src/` tree.

## System overview

Every namespace and sub-namespace has:
- **`ai-index.json`** — machine-readable index mapping files and sub-namespaces with short responsibility descriptions
- **`docs/`** — markdown files with detailed per-file or per-namespace documentation

The root `ai-index.json` maps all top-level namespaces. Each namespace index maps its files and sub-namespaces, which have their own indexes recursively.

## Index format

### Root index (`ai-index.json`)
```json
{
  ...
  "namespaces": [
    {
      "namespace": "<Name>",
      "fqcn": "Ineersa\\AgentCore\\<Name>",
      "path": "src/<Name>",
      "description": "<1-2 line summary>",
      "index": "src/<Name>/ai-index.json"
    }
  ]
}
```

### Namespace index (`src/<NS>/ai-index.json`)
```json
{
  "spec": "agent-core.ai-docs/v1",
  "namespace": "<NS>",
  ...
  "files": [
    {
      "file": "<ClassName>.php",
      "type": "class|interface|enum|readonly class|abstract class",
      "responsibility": "<1-2 line summary>",
      "docs": "docs/<ClassName>.md"
    }
  ],
  "subNamespaces": [
    {
      "namespace": "<SubNS>",
      "fqcn": "Ineersa\\AgentCore\\<NS>\\<SubNS>",
      "path": "<SubNS>",
      "description": "<1-2 line summary>",
      "index": "<SubNS>/ai-index.json"
    }
  ]
}
```
...
3. From the report, write/update:
   - `ai-index.json` — file entries with responsibility + docs reference
   - `docs/<File>.md` or `docs/overview.md` — detailed documentation
4. Validate JSON with `python3 -c "import json; json.load(open('<path>'))"`.

### Add a new file to existing namespace

1. Read the namespace's `ai-index.json` to understand current structure.
2. Launch scout for just that file.
3. Add a `files` entry in the index + create `docs/<File>.md`.
4. Update `updatedAt` in the index.

### Add a new sub-namespace

1. Create the sub-namespace directory with `ai-index.json` and `docs/`.
2. Add a `subNamespaces` entry in the parent's `ai-index.json`.
3. Launch scout for the new sub-namespace, wait for report, populate index + docs.

### Remove/rename a file

1. Update the namespace `ai-index.json` (remove/rename the entry).
...
1. Compare `ai-index.json` files entries against actual PHP files in the directory.
...
After any change:
- [ ] All `ai-index.json` files are valid JSON
...
- [ ] Every `subNamespaces` entry has a valid `index` path pointing to an existing `ai-index.json`
